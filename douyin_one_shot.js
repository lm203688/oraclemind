...
(async () => {
  ...
  console.log('登录:', loginCheck.status_code === 0 ? '✅' : '❌ ' + loginCheck.status_msg);
  if (loginCheck.status_code !== 0) { await browser.close(); return; }
  ...
  for (let i = 0; i < 30; i++) {
    await page.waitForTimeout(5000);
    if (await page.$('video')) { console.log('✅ 视频上传完成'); break; }
  ...
  if (!createResult) {
    console.log('⚠️ 未捕获到create API响应，检查页面状态...');
  }

  // Handle any confirmation dialogs
  try { await page.click('button:has-text("确认")', { timeout: 3000 }); console.log('✅ 确认'); } catch(e) {}
  try { await page.click('.semi-button-primary', { timeout: 2000 }); } catch(e) {}
  ...
  console.log('页面内容:', finalText.substring(0, 200));

  if (finalText.includes('发布成功') || finalText.includes('审核') || createResult?.includes('"status_code":0')) {
    console.log('🎉🎉🎉 视频发布成功！');
  } else {
    console.log('⚠️ 发布状态不确定，请检查抖音APP');
  }

  await page.screenshot({ path: '/home/z/my-project/final_publish.png' });
  await browser.close();
})();
