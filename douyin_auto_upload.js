...
(async () => {
  ...
  console.log('登录状态:', userInfo.status_code === 0 ? '✅ 已登录' : '❌ 未登录');
  if (userInfo.status_code !== 0) {
    console.log('错误:', userInfo.status_msg);
    await browser.close();
    return;
  }
  
  // Step 2: Go to publish page
  console.log('2. 打开发布页面...');
  ...
  console.log('3. 上传视频文件...');
  const videoPath = '/home/z/my-project/gene_tech_video/gene_tech_v2_small.mp4';
  const fileInput = await page.$('input[type=file]');
  if (fileInput) {
    await fileInput.setInputFiles(videoPath);
    console.log('✅ 视频文件已选择');
  } else {
    console.log('❌ 没找到文件上传框');
    await browser.close();
    return;
  }
  
  // Wait for upload to complete
  console.log('4. 等待视频上传...');
  await page.waitForTimeout(15000);
  
  // Take screenshot to check status
  await page.screenshot({ path: '/home/z/my-project/upload_status.png' });
  console.log('截图已保存');
  
  // Step 5: Fill in title and description
  console.log('5. 填写标题和描述...');
  
  // Try to find and fill title input
  const titleInput = await page.$('input[placeholder*="标题"], input[placeholder*="填写"], input[placeholder*="作品标题"]');
  if (titleInput) {
    await titleInput.click();
    await titleInput.fill('基因技术前沿速递｜2025年5月19日');
    console.log('✅ 标题已填写');
  } else {
  ...
  const descInput = await page.$('textarea[placeholder*="描述"], textarea[placeholder*="话题"], textarea[placeholder*="添加"], div[contenteditable=true]');
  if (descInput) {
    await descInput.click();
    await descInput.fill('基因技术前沿速递｜2025年5月19日 #基因技术 #科技前沿 #基因编辑 #生物科技');
    console.log('✅ 描述已填写');
  } else {
  ...
  await page.screenshot({ path: '/home/z/my-project/publish_ready.png' });
  console.log('发布前截图已保存');
  
  // Step 6: Click publish button
  console.log('6. 点击发布按钮...');
  const publishBtn = await page.$('button:has-text("发布"), div:has-text("发布")');
  if (publishBtn) {
    await publishBtn.click();
    console.log('✅ 已点击发布');
    await page.waitForTimeout(5000);
    await page.screenshot({ path: '/home/z/my-project/publish_result.png' });
