...
(async () => {
  ...
  // Monitor upload-related network requests
  page.on('request', req => {
  ...
  page.on('response', async res => {
    if (res.url().includes('upload') || res.url().includes('video')) {
      try {
        const ct = res.headers()['content-type'] || '';
        if (ct.includes('json')) {
          const data = await res.text().catch(() => '');
          console.log('RESP:', res.url().substring(0, 100), '->', data.substring(0, 200));
        }
      } catch(e) {}
    }
  });
  ...
  // Click the upload button div to trigger filechooser
  console.log('点击上传按钮...');
  const [fileChooser] = await Promise.all([
    page.waitForEvent('filechooser', { timeout: 10000 }),
    page.click('.upload-btn-PdfuUv')  // The upload button div
  ]);
  
  await fileChooser.setFiles('/home/z/my-project/gene_tech_video/gene_tech_v2_small.mp4');
  console.log('✅ 文件已选择');

  // Wait and monitor upload progress
  for (let i = 0; i < 12; i++) {
    await page.waitForTimeout(5000);
    const status = await page.evaluate(() => {
      const video = document.querySelector('video');
      const progress = document.querySelector('[class*=progress]');
      const uploadCard = document.querySelector('[class*=upload-card]');
      const successEl = document.querySelector('[class*=success]');
      const errorEl = document.querySelector('[class*=error], [class*=fail]');
      return {
        hasVideo: !!video,
        hasProgress: !!progress,
        progressText: progress ? progress.textContent.trim().substring(0, 50) : '',
        uploadCardText: uploadCard ? uploadCard.textContent.trim().substring(0, 100) : '',
        hasSuccess: !!successEl,
        hasError: !!errorEl,
        errorText: errorEl ? errorEl.textContent.trim().substring(0, 100) : ''
      };
    });
    console.log(`[${i*5}s]`, JSON.stringify(status));
    
    if (status.hasVideo || status.hasSuccess) {
      console.log('🎉 视频上传成功！');
      break;
    }
    if (status.hasError) {
      console.log('❌ 上传出错:', status.errorText);
      break;
    }
  }

  await page.screenshot({ path: '/home/z/my-project/upload_final2.png' });
  await browser.close();
})();
