...
(async () => {
  ...
  console.log('6. 点击发布...');
  const publishBtn = await page.$('button.publish-btn-PdfuUv, button:has-text("发布")');
  if (publishBtn) {
    const isEnabled = await publishBtn.isEnabled();
    console.log('发布按钮状态:', isEnabled ? '可用' : '不可用');
    if (isEnabled) {
      ...
      if (resultText.includes('发布成功') || resultText.includes('审核')) {
        console.log('🎉🎉🎉 视频发布成功！');
      } else {
        console.log('发布结果:', resultText.substring(0, 200));
      }
      await page.screenshot({ path: '/home/z/my-project/publish_success.png' });
    } else {
      console.log('❌ 发布按钮不可用，可能视频还在处理中');
      await page.screenshot({ path: '/home/z/my-project/publish_waiting.png' });
    }
  }
