...
(async () => {
  const userDataDir = '/home/z/my-project/browser_data8';
  ...
  // Take screenshot
  await page.screenshot({ path: '/home/z/my-project/scan_v3.png' });
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (url.includes('creator-micro') || url.includes('home')) {
      ...
      for (let k = 0; k < 30; k++) {
        ...
        if (k === 29) { console.log('TIMEOUT'); await context.close(); return; }
      }
      ...
      await page.screenshot({ path: '/home/z/my-project/scan_v3_result.png' });
      console.log('DONE');
      await context.close();
      return;
    }
    if (i % 6 === 0) console.log('WAIT', i * 5, 's');
  }
  console.log('TIMEOUT');
  await context.close();
