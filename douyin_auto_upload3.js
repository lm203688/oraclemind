...
(async () => {
  ...
  // Close popup
  try { await page.click('button:has-text("我知道了")', { timeout: 3000 }); } catch(e) {}
  try { await page.click('button:has-text("继续编辑")', { timeout: 3000 }); } catch(e) {}
  await page.waitForTimeout(1000);

  // Use filechooser event - click the upload button which triggers file dialog
  console.log('2. 点击上传按钮触发文件选择...');
  const [fileChooser] = await Promise.all([
    page.waitForEvent('filechooser', { timeout: 10000 }),
    page.click('text=点击上传') // or the upload area
  ]);
  
  console.log('3. 选择视频文件...');
  await fileChooser.setFiles('/home/z/my-project/gene_tech_video/gene_tech_v2_small.mp4');
  console.log('✅ 文件已通过filechooser选择');
  ...
  // Check status
  await page.screenshot({ path: '/home/z/my-project/upload_status3.png' });
  const hasVideo = await page.$('video');
  const hasProgress = await page.$('[class*=progress]');
  console.log('有视频预览:', !!hasVideo);
  console.log('有进度条:', !!hasProgress);
  ...
  if (hasVideo) {
    ...
    if (descEl) {
      await descEl.click();
      await page.keyboard.type('基因技术前沿速递｜2025年5月19日 #基因技术 #科技前沿 #基因编辑 #生物科技');
    }
    ...
    await page.waitForTimeout(5000);
    await page.screenshot({ path: '/home/z/my-project/publish_result3.png' });
    console.log('✅ 发布完成');
  } else {
    console.log('❌ 视频未上传成功');
  }
