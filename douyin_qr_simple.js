...
(async () => {
  const userDataDir = '/home/z/my-project/browser_data5';
  ...
  await page.screenshot({ path: '/home/z/my-project/qr_fullpage.png', fullPage: true });
  console.log('FULLPAGE_SAVED');
  
  // Also take viewport screenshot
  await page.screenshot({ path: '/home/z/my-project/qr_viewport.png' });
  console.log('VIEWPORT_SAVED');
  
  // Find ALL elements that could be QR codes
  const elements = await page.evaluate(() => {
    const results = [];
    // Check all images
    document.querySelectorAll('img').forEach(img => {
      const rect = img.getBoundingClientRect();
      results.push({tag:'img', src: img.src.substring(0,100), x:rect.x, y:rect.y, w:rect.width, h:rect.height});
    });
    // Check all canvases
    document.querySelectorAll('canvas').forEach(c => {
      const rect = c.getBoundingClientRect();
      results.push({tag:'canvas', x:rect.x, y:rect.y, w:rect.width, h:rect.height});
    });
    // Check all svgs
    document.querySelectorAll('svg').forEach(s => {
      const rect = s.getBoundingClientRect();
      results.push({tag:'svg', x:rect.x, y:rect.y, w:rect.width, h:rect.height});
    });
    // Check elements with qrcode in class or id
    document.querySelectorAll('[class*="qr"], [class*="QR"], [id*="qr"], [id*="QR"]').forEach(el => {
      const rect = el.getBoundingClientRect();
      results.push({tag:el.tagName, class:el.className.substring(0,50), x:rect.x, y:rect.y, w:rect.width, h:rect.height});
    });
    return results;
  });
