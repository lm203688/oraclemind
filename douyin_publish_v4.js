...
(async () => {
  ...
  // Go to publish page and wait longer
  console.log('1. 打开发布页面...');
  await page.goto('https://creator.douyin.com/creator-micro/content/publish/video/', { waitUntil: 'networkidle', timeout: 30000 });
  await page.waitForTimeout(8000);

  // Handle all popups
  try { await page.click('button:has-text("放弃")', { timeout: 2000 }); console.log('✅ 放弃草稿'); } catch(e) {}
  try { await page.click('button:has-text("继续编辑")', { timeout: 2000 }); console.log('✅ 继续编辑'); } catch(e) {}
  try { await page.click('button:has-text("我知道了")', { timeout: 2000 }); } catch(e) {}
  await page.waitForTimeout(3000);

  // Check current state
  const hasVideo = await page.$('video');
  const hasFileInput = await page.$('input[accept*="video"]');
  console.log('视频:', !!hasVideo, '文件框:', !!hasFileInput);

  if (hasVideo) {
    console.log('2. 视频已存在（之前的草稿）');
  } else if (hasFileInput) {
    console.log('2. 上传视频...');
    await hasFileInput.setInputFiles('/home/z/my-project/gene_tech_video/gene_tech_v2_small.mp4');
    await page.evaluate(() => {
    ...
    console.log('❌ 页面未正确加载');
    await page.screenshot({ path: '/home/z/my-project/debug.png' });
    await browser.close();
    return;
  }

  // Fill title and description
  console.log('3. 填写标题和描述...');
  ...
  // Close ALL popups
  console.log('4. 关闭所有弹窗...');
  for (let attempt = 0; attempt < 5; attempt++) {
    try { await page.click('button:has-text("我知道了")', { timeout: 1000 }); console.log('  关闭"我知道了"'); } catch(e) {}
    try { await page.click('.semi-modal-close', { timeout: 500 }); console.log('  关闭modal'); } catch(e) {}
  ...
  for (let i = 0; i < 20; i++) {
    ...
    try { await page.click('.semi-button-primary:has-text("确认")', { timeout: 500 }); } catch(e) {}
  ...
  if (!createResult) {
    console.log('⚠️ 未捕获到API响应，截图保存');
  }

  await page.screenshot({ path: '/home/z/my-project/final_v4.png' });
  await browser.close();
})();
