...
(async () => {
  ...
  // Go to creator platform (will redirect to login)
  await page.goto('https://creator.douyin.com/', { waitUntil: 'domcontentloaded', timeout: 20000 });
  await page.waitForTimeout(5000);
  
  // Screenshot the login page (QR code is shown by default)
  await page.screenshot({ path: '/home/z/my-project/scan_qr.png' });
  console.log('QR_READY');
  
  // Wait for user to scan QR code (up to 5 minutes)
  for (let i = 0; i < 60; i++) {
    await page.waitForTimeout(5000);
    const url = page.url();
