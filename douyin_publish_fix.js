...
(async () => {
  ...
  console.log('1. 关闭所有弹窗...');
  // Click "继续编辑" to resume the previous video
  try { await page.click('button:has-text("继续编辑")', { timeout: 3000 }); console.log('✅ 继续编辑'); } catch(e) {}
  await page.waitForTimeout(2000);
  
  // Close "我知道了" popup
  try { await page.click('button:has-text("我知道了")', { timeout: 2000 }); console.log('✅ 关闭我知道了'); } catch(e) {}
  await page.waitForTimeout(1000);
  
  // Close any semi-modal popups
  try { await page.click('.semi-modal-close, .semi-icon-close', { timeout: 2000 }); } catch(e) {}
  try { await page.click('[class*=close]', { timeout: 1000 }); } catch(e) {}
  await page.waitForTimeout(1000);

  // Check if video exists
  const hasVideo = await page.$('video');
  console.log('2. 视频状态:', !!hasVideo ? '已上传' : '不存在');
  ...
  if (!hasVideo) {
    ...
    await page.evaluate(() => {
      document.querySelector('input[accept*="video"]').dispatchEvent(new Event('change', { bubbles: true }));
    });
    for (let i = 0; i < 30; i++) {
      await page.waitForTimeout(5000);
  ...
  // Fill title and description if empty
  const titleVal = await page.evaluate(() => document.querySelector('input[placeholder*="标题"]')?.value || '');
  if (!titleVal) {
    await page.fill('input[placeholder*="标题"]', '基因技术前沿速递｜2025年5月19日');
  }
  const descText = await page.evaluate(() => document.querySelector('div[contenteditable=true]')?.textContent?.trim() || '');
  ...
  console.log('3. ✅ 标题和描述已就绪');

  await page.waitForTimeout(2000);

  // Close any remaining popups before clicking publish
  console.log('4. 再次关闭弹窗...');
  try { await page.click('button:has-text("我知道了")', { timeout: 2000 }); } catch(e) {}
  try { await page.click('.semi-modal-close, .semi-icon-close', { timeout: 1000 }); } catch(e) {}
  // Close hotspot association popup
  try { await page.click('[class*=semi-modal] [class*=close]', { timeout: 1000 }); } catch(e) {}
  try { await page.click('.semi-portal-inner [class*=close]', { timeout: 1000 }); } catch(e) {}
  await page.waitForTimeout(1000);
  ...
  // Wait for create API or page change
  for (let i = 0; i < 15; i++) {
    await page.waitForTimeout(3000);
    
    // Close any new popups that appear after clicking publish
    try { await page.click('button:has-text("确认")', { timeout: 1000 }); console.log('✅ 确认发布'); } catch(e) {}
    try { await page.click('.semi-button-primary', { timeout: 500 }); } catch(e) {}
    
    if (createResult) {
      console.log('🎉 发布API返回:', createResult.substring(0, 500));
      break;
    }
    
    const currentText = await page.evaluate(() => document.body.innerText.substring(0, 200));
    if (currentText.includes('发布成功') || currentText.includes('审核中')) {
      console.log('🎉🎉🎉 发布成功！');
      break;
    }
  }
  ...
  if (!createResult) {
    console.log('⚠️ 未捕获到API响应');
    // Try clicking publish again - maybe popup was blocking
    console.log('6. 再次尝试发布...');
    try { await page.click('button:has-text("我知道了")', { timeout: 1000 }); } catch(e) {}
    await page.waitForTimeout(1000);
    await page.click('button.primary-cECiOJ');
  ...
  await page.screenshot({ path: '/home/z/my-project/final_publish2.png' });
  console.log('截图已保存');
  await browser.close();
})();
