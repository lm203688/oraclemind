...
(async () => {
  ...
  // Go to publish page - DON'T click "继续编辑", click "放弃" to start fresh
  ...
  // Click "放弃" to discard previous draft and start fresh
  console.log('1. 放弃之前的草稿...');
  try { await page.click('button:has-text("放弃")', { timeout: 3000 }); console.log('✅ 已放弃'); } catch(e) { console.log('无草稿弹窗'); }
  ...
  await page.evaluate(() => {
    ...
    document.querySelector('input[accept*="video"]').dispatchEvent(new Event('input', { bubbles: true }));
  });
  ...
  // Close ALL popups before publishing
  console.log('5. 关闭弹窗...');
  // Close "我知道了" (共创中心提示)
  try { await page.click('button:has-text("我知道了")', { timeout: 2000 }); console.log('✅ 关闭共创提示'); } catch(e) {}
  await page.waitForTimeout(500);
  // Close semi-modal popups (热点关联等)
  ...
  // Wait and handle post-publish dialogs
  for (let i = 0; i < 20; i++) {
    await page.waitForTimeout(3000);
  ...
  await page.screenshot({ path: '/home/z/my-project/final_v3.png' });
  console.log('截图已保存');
  await browser.close();
})();
