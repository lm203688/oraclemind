...
(async () => {
  ...
  // Close all popups - click "继续编辑" to resume the previous upload
  console.log('1. 处理弹窗...');
  try { await page.click('button:has-text("继续编辑")', { timeout: 3000 }); console.log('✅ 继续编辑上次视频'); } catch(e) {}
  try { await page.click('button:has-text("我知道了")', { timeout: 2000 }); } catch(e) {}
  await page.waitForTimeout(3000);

  // Check if video is already uploaded
  let hasVideo = await page.$('video');
  if (hasVideo) {
    console.log('2. 视频已存在，跳过上传');
  } else {
    console.log('2. 上传视频...');
    ...
    for (let i = 0; i < 24; i++) {
      await page.waitForTimeout(5000);
      hasVideo = await page.$('video');
      if (hasVideo) { console.log('✅ 视频上传完成'); break; }
    }
  }
  ...
  await page.waitForTimeout(3000);
  await page.screenshot({ path: '/home/z/my-project/before_publish.png' });

  // Click publish - look for the correct button
  console.log('5. 查找并点击发布按钮...');
  ...
  if (publishBtn) {
    ...
    console.log('发布按钮:', isEnabled ? '可用' : '不可用');
    if (isEnabled) {
      ...
      try { await page.click('button:has-text("确认")', { timeout: 3000 }); console.log('✅ 确认发布'); } catch(e) {}
      try { await page.click('button:has-text("确定")', { timeout: 2000 }); } catch(e) {}
      
      await page.waitForTimeout(5000);
      const resultText = await page.evaluate(() => document.body.innerText.substring(0, 800));
      console.log('页面内容:', resultText.substring(0, 300));
      
      if (resultText.includes('发布成功') || resultText.includes('审核中') || resultText.includes('已发布')) {
        console.log('🎉🎉🎉 视频发布成功！');
      }
      
      await page.screenshot({ path: '/home/z/my-project/publish_done.png' });
    }
  }
