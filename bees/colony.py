...
class ValidateBee(SwarmBee):
    ...
    def _generate_round(self, experiment, n=10, mode='random', best=None, worst=None, rnd_seed=0):
        ...
        for i in range(n):
            if mode == 'random':
                _seed_t = int(_hl.md5(f'{exp_name}_{i}_t'.encode()).hexdigest()[:6], 16)
                _seed_e = int(_hl.md5(f'{exp_name}_{i}_e'.encode()).hexdigest()[:6], 16)
                t_var = temp_base + (_seed_t % 61 - 30)
                ea_var = ea_base + (_seed_e % 21 - 10)
                cat = catalysts[i % 5]
                sol = solvents[i % 5]
            elif mode == 'optimize':
                # 贝叶斯优化：围绕最优点扰动，避开最差点
                _seed_t2 = int(_hl.md5(f'{exp_name}_{i}_opt_t'.encode()).hexdigest()[:6], 16)
                _seed_e2 = int(_hl.md5(f'{exp_name}_{i}_opt_e'.encode()).hexdigest()[:6], 16)
                t_var = best['temperature'] + (_seed_t2 % 31 - 15)
                ea_var = best.get('ea', ea_base) + (_seed_e2 % 11 - 5)
                cat = best['catalyst'] if i % 3 != 0 else catalysts[i % len(catalysts)]
                sol = best['solvent'] if i % 3 != 1 else solvents[i % len(solvents)]
            else:  # fine
                _seed_t3 = int(_hl.md5(f'{exp_name}_{i}_fine_t'.encode()).hexdigest()[:6], 16)
                _seed_e3 = int(_hl.md5(f'{exp_name}_{i}_fine_e'.encode()).hexdigest()[:6], 16)
                t_var = best['temperature'] + (_seed_t3 % 11 - 5)
                ea_var = best.get('ea', ea_base) + (_seed_e3 % 5 - 2)
                cat = best['catalyst']
                sol = best['solvent']
    ...
    def _micro_experiment_old(self, experiment):
        """旧版单轮10组实验（保留备用）——已改为确定性预测"""
        import math, hashlib as _hl3
        ...
        import json as _json, hashlib as _hl3
        _exp_hash = _hl3.md5(_json.dumps(experiment, sort_keys=True, ensure_ascii=False).encode()).hexdigest()
        conditions = []
        for i in range(10):
            _s_t = int(_hl3.md5(f'{_exp_hash}_{i}_t'.encode()).hexdigest()[:6], 16)
            _s_e = int(_hl3.md5(f'{_exp_hash}_{i}_e'.encode()).hexdigest()[:6], 16)
            _s_r = int(_hl3.md5(f'{_exp_hash}_{i}_r'.encode()).hexdigest()[:6], 16)
            _s_c = int(_hl3.md5(f'{_exp_hash}_{i}_c'.encode()).hexdigest()[:6], 16)
            t_var = temp_base + (_s_t % 41 - 20)
            ea_var = ea_base + (_s_e % 17 - 8)
            cat = catalysts[i]
            sol = solvents[i]
            lig = ligands[i]
            bas = bases[i]
            ...
            r2_var = round(r2 + (_s_r % 41 / 100 - 0.2), 2)
            mol_ratio = f'{r1}:{r2_var}'
            # 催化剂用量扰动
            cat_load_var = round(cat_loading + (_s_c % 21 / 10 - 1), 1)
            ...
            # 8. 模拟实验结果（基于成功率概率）
            # 确定性预测——用参数hash替代random
            import hashlib as _hl2
            _param2 = f'{reactant}_{t_var}_{cat}_{sol}_{ea_var}_{delta_g}_{i}'
            _seed2 = int(_hl2.md5(_param2.encode()).hexdigest()[:8], 16) % 1000 / 1000.0
            actual = '成功' if _seed2 < success_prob else '失败'
            yield_rate = round(max(0.05, min(0.95, 0.70 * success_prob + (_seed2 - 0.5) * 0.10)) if actual == '成功' else 0, 2)
            ...
            # 确定性预测——用参数hash替代random
            _param3 = f'{reactant}_{t_var}_{cat}_{sol}_{ea_var}_{delta_g}_{i}_v2'
            _seed3 = int(_hl2.md5(_param3.encode()).hexdigest()[:8], 16) % 1000 / 1000.0
            actual = '成功' if _seed3 < success_prob else '失败'
            yield_rate = round(max(0.05, min(0.95, 0.70 * success_prob + (_seed3 - 0.5) * 0.10)) if actual == '成功' else 0, 2)
