...
(async () => {
  ...
  // Monitor publish API calls
  page.on('response', async res => {
    if (res.url().includes('create') || res.url().includes('publish') || res.url().includes('post')) {
      try {
        const ct = res.headers()['content-type'] || '';
        if (ct.includes('json')) {
          const data = await res.text().catch(() => '');
          console.log('PUBLISH_API:', res.url().substring(0, 100), '->', data.substring(0, 300));
        }
      } catch(e) {}
    }
  });
  ...
  // Click "继续编辑" to resume previous upload
  console.log('1. 恢复之前的视频...');
  try { await page.click('button:has-text("继续编辑")', { timeout: 3000 }); console.log('✅ 继续编辑'); } catch(e) { console.log('没有继续编辑弹窗'); }
  ...
  // Click the correct publish button (not "高清发布")
  console.log('3. 点击发布按钮...');
  // The "发布" button has class "primary-cECiOJ fixed-J9O8Yw"
  ...
  if (publishBtn) {
    ...
    try { await page.click('button.semi-button-primary', { timeout: 2000 }); console.log('✅ 点击确认按钮'); } catch(e) {}
    ...
    console.log('页面内容:', text.substring(0, 200));
