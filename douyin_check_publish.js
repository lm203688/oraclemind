...
(async () => {
  ...
  // Monitor ALL API responses during publish
  page.on('response', async res => {
    const url = res.url();
    if (url.includes('create') || url.includes('publish') || url.includes('aweme')) {
      try {
        const ct = res.headers()['content-type'] || '';
        if (ct.includes('json')) {
          const data = await res.text().catch(() => '');
          if (data.length < 1000) {
            console.log('API:', url.substring(0, 100), '->', data.substring(0, 300));
          }
        }
      } catch(e) {}
    }
  ...
  // Handle popups - click "继续编辑" to resume
  try { await page.click('button:has-text("继续编辑")', { timeout: 3000 }); console.log('✅ 继续编辑'); } catch(e) { console.log('无继续编辑弹窗'); }
  ...
  if (hasVideo) {
    ...
    // Now click publish and wait for the actual API response
    console.log('点击发布...');
    
    // Set up a promise to catch the create API response
    const createResponsePromise = new Promise(resolve => {
      page.on('response', async res => {
        if (res.url().includes('aweme/create') || res.url().includes('media/aweme/create')) {
          const data = await res.text().catch(() => '');
          resolve({ url: res.url(), status: res.status(), data });
        }
      });
    });

    await page.click('button.primary-cECiOJ');
    console.log('✅ 已点击发布，等待API响应...');
    ...
    console.log('发布API响应:', JSON.stringify(createResp).substring(0, 500));
    
    await page.waitForTimeout(5000);
    await page.screenshot({ path: '/home/z/my-project/publish_api_result.png' });
  } else {
    console.log('❌ 视频不存在，需要重新上传');
  }
