...
server.tool('aishield_prompt_check', '🛡️ Prompt安全检测 — 检测用户输入的Prompt是否存在注入/越狱/数据外传风险。支持中文/英文 (0.5 ATEX/call)', {
  prompt: z.string().describe('待检测的Prompt文本（至少10个字符）'),
}, async ({ prompt }) => {
  try {
    const resp = await fetch('http://127.0.0.1:8450/api/v1/prompt-check', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'User-Agent': 'ATEX-MCP-Server/1.0' },
    ...
    const data = await resp.json();
    return { content: [{ type: 'text' as const, text: `Prompt安全检测:\n安全: ${data.safe ? '✅' : '❌'}\n评分: ${data.score}/100\n风险: ${data.risk}\n\n${data.summary}\n\n发现: ${JSON.stringify(data.findings, null, 2)}` }] };
  } catch (e: any) {
