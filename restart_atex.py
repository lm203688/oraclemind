import json, requests
# 重启8420服务
cmd = "pkill -f 'python.*server.py' 2>/dev/null; sleep 1; cd /home/ubuntu/atex && nohup python3 -m api.server > /tmp/atex.log 2>&1 & sleep 2; curl -s -o /dev/null -w '%{http_code}' http://localhost:8420/api/v1/status"
payload = {
    "model": "xiaowu-agent",
    "messages": [{"role":"user","content":f"执行: {cmd}"}],
    "temperature": 0
}
r = requests.post("http://150.158.119.19:3003/v1/chat/completions",
    headers={"Authorization":"Bearer xiaowu-internal-2026","Content-Type":"application/json"},
    json=payload, timeout=50)
d = r.json()
print(d['choices'][0]['message']['content'])
