...
(async () => {
  ...
  // Go to publish page first to load the upload SDK
  ...
  // Now use the page's own upload mechanism - inject the file into the byted-uploader
  console.log('通过页面SDK上传视频...');
  const result = await page.evaluate(async () => {
    // Find the upload component instance
    const videoInput = document.querySelector('input[accept*="video"]');
    if (!videoInput) return { error: 'no video input' };
    
    // The input has a React fiber that connects to the upload component
    // Let's try to trigger it properly
    const reactKey = Object.keys(videoInput).find(k => k.startsWith('__reactFiber') || k.startsWith('__reactInternalInstance'));
    if (!reactKey) return { error: 'no react fiber', keys: Object.keys(videoInput).filter(k => k.startsWith('__')).join(',') };
    
    return { found: true, reactKey };
  });
  console.log('React fiber:', JSON.stringify(result));

  // Alternative: use CDP to set files on the input
  const videoInput = await page.$('input[accept*="video"]');
  if (videoInput) {
    // Use CDP method to upload files
    await videoInput.setInputFiles('/home/z/my-project/gene_tech_video/gene_tech_v2_small.mp4');
    console.log('✅ setInputFiles done');
    
    // Dispatch events that React/Upload SDK listens to
    await page.evaluate(() => {
  ...
  // Check for upload progress or video preview
  const status = await page.evaluate(() => {
    const video = document.querySelector('video');
    const allText = document.body.innerText;
    const hasUploadProgress = allText.includes('上传中') || allText.includes('上传成功') || allText.includes('%');
    const hasError = allText.includes('上传失败') || allText.includes('格式不支持');
    return {
      hasVideo: !!video,
      hasUploadProgress,
      hasError,
  ...
  console.log('Status:', JSON.stringify(status).substring(0, 300));

  await page.screenshot({ path: '/home/z/my-project/upload_cdp.png' });
  await browser.close();
})();
